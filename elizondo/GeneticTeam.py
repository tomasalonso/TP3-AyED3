# -*- coding: utf-8 -*-
from constants import MOVIMIENTO, A, LEFT
...
class GeneticTeam:
    ...
    def startGame(self, columns, rows, steps, side, players, oponent_players):
        ...
        if self.team == A:
            self.logicalBoard = LogicalBoard(columns, rows, players,
                                             oponent_players)
        else:
            self.logicalBoard = LogicalBoard(columns, rows, oponent_players,
                                             players)
    ...
    def move(self, board_state):
        ...
        # Si quiero considerar todas las posibilidades, tengo que usar la
        # linea comentada
        ...
        # Miro la mejor jugada asumiendo que el oponente se mueve siempre lo
        # peor posible respecto a mi jugada
        for team_move in team_moves:
            if not self.logicalBoard.isValidTeamMove(
                    self.logicalBoard[self.team], team_move):
                continue

            move_value = None
            for oponent_move in oponent_moves:
                if not self.logicalBoard.isValidTeamMove(
                        self.logicalBoard[self.oponent_team], oponent_move):
                    continue
        ...
        # El jugador es medio random, pero esto no es algo que los jugadores
        # genéticos deban considerar Es solo para que sirva mejor como jugador
        # de debuggin
    ...
    def evaluateBoard(self):
        ...
        # Distancia maxima entre dos objetos en el tablero, sirve para
        # normalizar
        ...
        rival_goal_distance = min([
            norm((g[0] - ball.i, g[1] - ball.j))
            for g in self.logicalBoard.getGoal(self.oponent_team)
        ])
        rival_goal_distance = float(
            max_distance - rival_goal_distance
        ) / max_distance  # Mas lejos mas bajo y normalizado entre 0 y 1

        goal_distance = min([
            norm((g[0] - ball.i, g[1] - ball.j))
            for g in self.logicalBoard.getGoal(self.team)
        ])
        goal_distance = float(
            max_distance - goal_distance
        ) / max_distance  # Mas lejos mas bajo y normalizado entre 0 y 1
        ...
        ball_distance = sum([
            norm((p.i - ball.i, p.j - ball.j))
            for p in self.logicalBoard[self.team]
        ])
        ball_distance = float(3 * max_distance - ball_distance) / float(
            3 * max_distance)

        rival_ball_distance = sum([
            norm((p.i - ball.i, p.j - ball.j))
            for p in self.logicalBoard[self.oponent_team]
        ])
        rival_ball_distance = float(
            max_distance - rival_ball_distance) / max_distance
        ...
        # Dispersion
        dispersion = sum([
            norm((p1.i - p2.i, p1.j - p2.j))
            for p1 in self.logicalBoard[self.team]
            for p2 in self.logicalBoard[self.team]
        ])
        ...
        distances_to_oponent = sum([
            norm((p1.i - p2.i, p1.j - p2.j))
            for p1 in self.logicalBoard[self.team]
            for p2 in self.logicalBoard[self.oponent_team]
        ])
        distances_to_oponent = float(distances_to_oponent) / float(
            3 * max_distance)
